...
function limit_login_admin_menu() {
	...
	// Modern WP?
	if (version_compare($wp_version, '3.0', '>=')) {
	    add_options_page(__('Limit Login Attempts', 'limit-login-attempts'), __('Limit Login Attempts', 'limit-login-attempts'), 'manage_options', 'limit-login-attempts', 'limit_login_option_page');
	    return;
	}

	// Older WPMU?
	if (function_exists("get_current_site")) {
	    add_submenu_page('wpmu-admin.php', __('Limit Login Attempts', 'limit-login-attempts'), __('Limit Login Attempts', 'limit-login-attempts'), 9, 'limit-login-attempts', 'limit_login_option_page');
	    return;
	}

	// Older WP
	add_options_page(__('Limit Login Attempts', 'limit-login-attempts'), __('Limit Login Attempts', 'limit-login-attempts'), 9, 'limit-login-attempts', 'limit_login_option_page');
}
